...
const Step1 = ({ onNext }) => {
    ...
    getVisitTypes()
      .then((res) => {
        ...
        api.forEach((it) => {
          ...
          m.set(code, {
            id: Number.isFinite(idNum) ? idNum : undefined, // pakai id API kalau numeric
            label,
            description,
          });
        });
        ...
        setVisitorTypes((prev) =>
          prev.map((def) => {
            ...
            return {
              ...
              // kalau API kirim deskripsi kosong, jangan timpa default
  ...
  const handleSubmit = () => {
    ...
    // Kirim ID numerik untuk backend + label untuk tampilan Step4
    onNext({
      jenisKunjungan: selected.id,             // <-- BIGINT OK
      jenisKunjunganLabel: selected.label,     // <-- untuk ditampilkan di Step4
    });
  };
